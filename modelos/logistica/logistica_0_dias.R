...
df %>%
  mutate(inadimplido_acima_0_dias_factor = ifelse(inadimplido_acima_0_dias > 0,
                        yes = "sim",
                        no = "nao"),
         inadimplido_acima_0_dias_factor = factor(inadimplido_acima_0_dias_factor)) -> df
df %>% kable() %>%  kable_styling(bootstrap_options = "striped", full_width = F, font_size = 25)
# Estatísticas descritivas univariadas da base de dados
summary(df)

# Tabela de frequências absolutas da variável atrasado
table(df$inadimplido_acima_0_dias_factor)
# Proporção de inadimplentes com apenas 0 dias frente a taxa usual de inadimplentes de 90 dias
print(paste(round(544/838 * 100, 2), "%"))
...
modelo_atrasos_0_dias <- glm(
    formula = inadimplido_acima_0_dias_factor ~ cnae_secao + indexador + modalidade + porte + 
...
step_atrasos_0_dias <- step(object = modelo_atrasos_0_dias,
                        k = qchisq(p = 0.05, df = 1, lower.tail = FALSE))
step_atrasos_0_dias
summary(modelo_atrasos_0_dias)
summary(step_atrasos_0_dias)
logLik(modelo_atrasos_0_dias)
logLik(step_atrasos_0_dias)
summ(model = modelo_atrasos_0_dias, confint = T, digits = 4, ci.width = 0.95)
summ(model = step_atrasos_0_dias, confint = T, digits = 4, ci.width = 0.95)
export_summs(modelo_atrasos_0_dias, scale = F, digits = 4)
export_summs(step_atrasos_0_dias, scale = F, digits = 4)
...
predict(object=step_atrasos_0_dias,
        df_predict,
        type = "response"
)
# Construção de uma matriz de confusão
df$phat_0 <- modelo_atrasos_0_dias$fitted.values
df$phat_step_0 <- step_atrasos_0_dias$fitted.values
...
cutoff <- 0.5
cutoff_2 <- .6
confusionMatrix(table(predict(modelo_atrasos_0_dias, type="response") >= cutoff_2,
                      df$inadimplido_acima_0_dias_factor == 'sim')[2:1, 2:1])

confusionMatrix(table(predict(step_atrasos_0_dias, type="response") >= cutoff_2,
                      df$inadimplido_acima_0_dias_factor == 'sim')[2:1, 2:1])
...
data.frame(Sensitividade = confusionMatrix(table(predict(modelo_atrasos_0_dias,
                                                         type = "response") >= 0.5,
                                                 df$inadimplido_acima_0_dias_factor == 'sim')[2:1, 2:1])[["byClass"]][["Sensitivity"]],
           Especificidade = confusionMatrix(table(predict(modelo_atrasos_0_dias,
                                                          type = "response") >= 0.5,
                                                  df$inadimplido_acima_0_dias_factor == 'sim')[2:1, 2:1])[["byClass"]][["Specificity"]],
           Acurácia = confusionMatrix(table(predict(modelo_atrasos_0_dias,
                                                    type = "response") >= 0.5,
                                            df$inadimplido_acima_0_dias_factor == 'sim')[2:1, 2:1])[["overall"]][["Accuracy"]]) %>%
  kable() %>%
  kable_styling(bootstrap_options = "striped", position = "center",
                full_width = F, 
                font_size = 27)

data.frame(Sensitividade = confusionMatrix(table(predict(step_atrasos_0_dias,
                                                         type = "response") >= 0.5,
                                                 df$inadimplido_acima_0_dias_factor == 'sim')[2:1, 2:1])[["byClass"]][["Sensitivity"]],
           Especificidade = confusionMatrix(table(predict(step_atrasos_0_dias,
                                                          type = "response") >= 0.5,
                                                  df$inadimplido_acima_0_dias_factor == 'sim')[2:1, 2:1])[["byClass"]][["Specificity"]],
           Acurácia = confusionMatrix(table(predict(step_atrasos_0_dias,
                                                    type = "response") >= 0.5,
                                            df$inadimplido_acima_0_dias_factor == 'sim')[2:1, 2:1])[["overall"]][["Accuracy"]]) %>%
  kable() %>%
  kable_styling(bootstrap_options = "striped", position = "center",
...
confusionMatrix(table(predict(modelo_atrasos_0_dias, type = "response") >= cutoff_03,
                      df$inadimplido_acima_0_dias_factor == 'sim')[2:1, 2:1])
confusionMatrix(table(predict(step_atrasos_0_dias, type = "response") >= cutoff_03,
                      df$inadimplido_acima_0_dias_factor == 'sim')[2:1, 2:1])
...
confusionMatrix(table(predict(modelo_atrasos_0_dias, type = "response") >= cutoff_07,
                      df$inadimplido_acima_0_dias_factor == 'sim')[2:1, 2:1])
confusionMatrix(table(predict(step_atrasos_0_dias, type = "response") >= cutoff_07,
                      df$inadimplido_acima_0_dias_factor == 'sim')[2:1, 2:1])


predicoes_0 <- prediction(predictions = modelo_atrasos_0_dias$fitted.values, 
                        labels = df$inadimplido_acima_0_dias_factor)
predicoes_0_step <- prediction(predictions = modelo_atrasos_0_dias$fitted.values, 
                          labels = df$inadimplido_acima_0_dias_factor) 
...
dados_curva_roc_0_step <- performance(predicoes_0_step, measure = "sens") 
...
sensitividade_0_step <- (performance(predicoes_0_step, measure = "sens"))@y.values[[1]] 
especificidade_0 <- (performance(predicoes_0, measure = "spec"))@y.values[[1]]
especificidade_0_step <- (performance(predicoes_0_step, measure = "spec"))@y.values[[1]]

#Extraindo os cutoffs:
cutoffs_0 <- dados_curva_roc_0@x.values[[1]]
cutoffs_0_step <- dados_curva_roc_0_step@x.values[[1]] 
...
dados_plotagem_0_step <- cbind.data.frame(cutoffs_0_step, especificidade_0_step, sensitividade_0_step)
...
#Plotando:
ggplotly(dados_plotagem_0_step %>%
           ggplot(aes(x = cutoffs_0_step, y = especificidade_0_step)) +
           geom_line(aes(color = "Especificidade"),
                     size = 1) +
           geom_point(color = "#95D840FF",
                      size = 1.9) +
           geom_line(aes(x = cutoffs_0_step, y = sensitividade_0_step, color = "Sensitividade"),
                     size = 1) +
           geom_point(aes(x = cutoffs_0_step, y = sensitividade_0_step),
                      color = "#440154FF",
                      size = 1.9) +
           labs(x = "Cutoff",
                y = "Sensitividade/Especificidade") +
...
ROC_0 <- roc(response = df$inadimplido_acima_0_dias_factor, 
...
ROC_0_step <- roc(response = df$inadimplido_acima_0_dias_factor, 
             predictor = step_atrasos_0_dias$fitted.values)
