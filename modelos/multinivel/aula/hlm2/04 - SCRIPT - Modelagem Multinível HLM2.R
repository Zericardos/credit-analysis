...
load(file = "modelos/multinivel/aula/hlm2/estudante_escola.RData")
...
#igual a 3.6 anos?
predict(modelo_final_hlm2, level = 0:1,  # os níveis separamos assim
        newdata = data.frame(escola = "1",  # escola é variável Factor, por isso
                             horas = 11,  # usamos aspas duplas
                             texp = 3.6))
...
estudante_escola %>%
  ...
  geom_smooth(aes(x = horas, y = fitted_escola,
                  color = factor(escola)),  # se suprimido color, teremos um
              method = "lm", se = F) +  # plot de OLS. Vê-se que é imbatível
  scale_colour_viridis_d() +  # a modelagem multinível
  labs(x = "Quantidade Semanal de Horas de Estudo do Aluno",
